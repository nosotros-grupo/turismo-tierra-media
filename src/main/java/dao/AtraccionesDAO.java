...
public class AtraccionesDAO {
	...
	public static LinkedList<Atraccion> findAll() throws SQLException {
		...
		while (resultados.next()) {
			Atraccion atracTemp = toAtraccion(resultados);
			atracciones.add(atracTemp);
			Gl.atraccionMap.put(resultados.getInt(6), toAtraccion(resultados));
		}
		conn.close();
		return atracciones;
	}

	// AtraccionDB = nombre, costo, tiempo, cupo, tipo, id
	private static Atraccion toAtraccion(ResultSet resultados) throws SQLException {
		return new Atraccion(
				...
				resultados.getInt(4), 
				TipoAtraccion.valueOf(resultados.getString(5).toUpperCase()),
				resultados.getInt(6));
	}
...
//		}
		for (Atraccion atra: atracciones) {
			statement.setInt(1, atra.getCupoDePersonasDiario());
			statement.setString(2, atra.getNombre());
			statement.executeUpdate();
		}
		conn.close();
	}
